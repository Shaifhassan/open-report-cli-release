...
When automating reports (e.g., via Task Scheduler or Cron), passing a plaintext password via the command line is a security risk. Open Report provides a dedicated utility to encrypt passwords into "Secure Tokens" that only your CLI installation can decrypt.

### 1. Generating a Secure Token

Use the `enc_password` utility to convert a plaintext password into an encrypted string.

```bash
open_report dump enc_password
...
- Output: `✅ Success: Encrypted Password: [SECURE_TOKEN_XXXXX]`

### 2. Using the Token in Automation
...
```bash
open_report dump -c PROD -q "SELECT..." -p resort="SAIV" -p password="[SECURE_TOKEN_XXXXX]" --initialize-db
...
> [!WARNING]
> **Encryption Context:** The `enc_password` command uses a system-defined salt. If you change your Master Password or move to a new machine, you will need to re-generate these tokens as the encryption context will have changed.
